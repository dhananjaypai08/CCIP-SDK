...
txn_hash = client.send_tokens_to_sender_contract("ethereum_sepolia", "CCIP-BnM", 0.5)
...
txn_hash = client.allow_destination_chain(current_chain="ethereum_sepolia", destination_chain="base_sepolia")
...
print(f"Deployed contract address: {contract}\n")
txn_hash = client.allow_source_chain(current_chain="base_sepolia", sender_chain="ethereum_sepolia")
print(f"Allowed sender chain ethereum_sepolia done with txnHash : {txn_hash}\n")
txn_hash = client.allow_sender_on_receiver(sender_chain="ethereum_sepolia", receiver_chain="base_sepolia")
print(f"Allowed the sender contract to send messages on reciever chain with txnHash : {txn_hash}\n")

txn_url = client.transfer(sender_chain="ethereum_sepolia", receiver_chain="base_sepolia", text="Hi dj boi", amount=0.1)
